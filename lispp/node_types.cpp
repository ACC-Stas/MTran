...
std::string Empty::ToString(int tab_number) const {
    auto tabs = std::string(tab_number, ' ');
    return tabs + "Empty ()\n";
}
...
std::string Const::ToString(int tab_number) const {
    auto tabs = std::string(tab_number, ' ');
    return tabs + "Const " + value_.ToString(tab_number + 2) + "\n";
}
...
std::string Var::ToString(int tab_number) const {
    auto tabs = std::string(tab_number, ' ');
    return tabs + "Var " + name_ + "\n";
}
...
std::string Quote::ToString(int tab_number) const {
    auto tabs = std::string(tab_number, ' ');
    return tabs + "Quote'" + value_->ToString();
}
...
std::string Pair::ToString(int tab_number) const {
    auto tabs = std::string(tab_number, ' ');
    std::string result = tabs + "Pair \n";
    auto elements = ToVector();
    auto last = elements.end();
    --last;
    for (auto el = elements.begin(); el != last; ++el){
        result += (*el)->ToString(tab_number + 2);
    }
    if ((*last)->Type() == NodeType::EMPTY){
        result.pop_back();
    } else {
        result += ". ";
        result += (*last)->ToString(tab_number + 2);
    }
    return result + "\n";
}
...
std::string Func::ToString(int tab_number) const {
    return "Func";
}
...
std::string FuncList::ToString(int tab_number) const {
    std::string result;
